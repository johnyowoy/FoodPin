...
class RestaurantTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let customFont = UIFont(name: "papyrus", size: 40.0) {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
        ...
        let checkInAction = UIAlertAction(title: checkActionTitle, style: .default, handler: {
            ...
            // Cell的右邊部分是預留給輔助視圖(AccessoryView)用的。
            // 有四種內建輔助試視圖
            // 「揭示只指示符號」(disclosure indicator), 「細節揭示按鈕」(detail disclosure button), 「打勾符號」(checkmark), 「細節符號」(detail)
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") {
            ...
            // 呼叫完成處理器來取消動作按鈕(設定完成處理器為true值)
            completionHandler(true)
        }
        // 自訂deleteAction Color
